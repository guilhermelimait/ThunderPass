...
private val BarToggleOnBg  = ColorProvider(Color(0xFFFFC107))
private val BarToggleOffBg = ColorProvider(Color(0xFF111111))
private val BarToggleOnFg  = ColorProvider(Color(0xFF1A1100))
private val BarToggleOffFg = ColorProvider(Color.White)

/**
 * 1h×4w profile bar widget — horizontal gradient bar showing avatar head,
 * display name, volts, passes, badges, and a thunder toggle button.
 */
class ProfileWidget : GlanceAppWidget() {
    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        provideContent { BarContent(data, isRunning) }
    }

    @Composable
    private fun BarContent(data: BarData, isRunning: Boolean) {
        Row(
            ...
            Box(
                modifier = GlanceModifier
                    .size(36.dp)
                    .cornerRadius(18.dp)
                    .background(if (isRunning) BarToggleOnBg else BarToggleOffBg)
                ...
                Image(
                    ...
                    colorFilter = ColorFilter.tint(
                        if (isRunning) BarToggleOnFg else BarToggleOffFg
                    ),
                )
            }
        }
    ...
    companion object {
        suspend fun refresh(context: Context) {
            ProfileWidget().updateAll(context)
        }
    }
}
