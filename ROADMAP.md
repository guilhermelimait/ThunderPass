...
Personal data (display name, greeting, avatar, stats) is only transferred during a GATT exchange and only to another verified ThunderPass device. If **Privacy Mode** is enabled, even the GATT payload is anonymized — the peer sees only `"Private User"` with no avatar or greeting.
...
| Layer | Scope | Window | Effect |
|---|---|---|---|
| **Scan-level** | Per `device.address` | **60 minutes** | Suppresses redundant GATT connections to the same hardware for the full rotating-ID window — aligns with the rate at which IDs change so the same physical device can never trigger more than one exchange per hour |
| **Rotating-ID** | Per rotating ID | 60 minutes | The same rotating ID cannot open more than one GATT exchange per hour |
| **Identity** | Per Supabase `userId` or installation ID | **24 hours** | Even if rotating IDs change, two users walking side by side all day earn **at most one Spark and one Volt reward per day**. For privacy-mode peers (no stable identity in payload), the MAC address is used as the 24-hour dedup key so anonymous devices cannot farm Volts every hour. Within the 24-hour window the existing pass's timestamp is refreshed so the gallery always shows the latest meeting time. |

This pyramid guarantees that the encounter list is a meaningful social diary, not a spam log.

---

### How the Server Recognises a User
No account or registration is required. On first launch ThunderPass automatically signs in **anonymously** via Supabase. This creates a server-side UUID tied to the session — invisible to the user, requiring no email or password. The flow is:

1. On first launch, `auth.signInAnonymously()` is called silently. Supabase issues a **UUID** (`userId`) and persists the session to SharedPreferences.
2. The session auto-refreshes every 30 days in the background — the user never notices.
3. A P-256 key pair is generated **inside the Android Keystore** (hardware-backed TEE on most devices). The public key is stored in Supabase under `profiles.public_key`; the private key never leaves the device.
4. The UUID and a time-bound ECDSA-P256 signature are embedded in the GATT payload (never in the BLE advertisement).
5. On receiving a GATT payload, the receiving device **verifies the UUID against the Supabase `profiles` table** before accepting the encounter. A UUID absent from the server is immediately rejected.
6. If the payload carries a `sig` field, the receiver fetches the sender's registered public key from `profiles.public_key` and verifies an ECDSA-SHA256 signature over `"thunderpass:v1:{userId}:{rotatingId}:{ts5min}"`. A valid signature proves the packet came from the device that generated the key pair — a copied UUID without the private key fails this check.
7. The server therefore recognises each device exactly once (per the 24-hour identity dedup window) and only after a Keystore-bound cryptographic GATT exchange.

If the app is offline, verification is skipped leniently (the encounter is accepted locally) but Volts are still subject to all local dedup rules.

> ⚠️ **Supabase setup required for full verification:** run `ALTER TABLE profiles ADD COLUMN public_key TEXT DEFAULT NULL;` in the Supabase SQL Editor once to enable the signing layer.
...
**In plain terms:**
- If you **uninstall and reinstall** on the same device without signing in: you get a fresh anonymous UUID and your Spark history is gone. Your cloud profile (name, avatar, Volts) is still on the server but cannot be automatically linked back — it stays orphaned until you sign in with an account.
- If you **factory reset**: same as above, plus your `ANDROID_ID` changes, so even the deterministic installation ID is different.
- If you **sign in with Google or email** before losing the app, your profile on the server is linked to that account and fully restored after reinstall.

> 📌 **Planned for a future version:** optional cloud backup of the Spark history, and the ability to link an anonymous session to a named account after the fact — so your pass diary is never permanently lost.
...
| Concern | Mitigation |
|---|---||
| **Passive tracking** | Rotating IDs change every hour; no static identifier in the advertisement |
| **Impersonation** | Claimed `userId` is verified against the live Supabase `profiles` table on every GATT exchange. The UUID alone is insufficient — it must be paired with a valid ownership proof. |
| **Cryptographic ownership** | Each device generates a P-256 key pair inside the **Android Keystore** (hardware TEE). The public key is stored in Supabase; the private key never leaves the device. Every GATT payload carries an ECDSA-SHA256 signature over `userId + rotatingId + ts5min`. Peers verify this signature — a copied UUID without the private key is rejected. |
| **Payload replay** | Signatures are bound to a 5-minute time window (`floor(epochSeconds / 300)`) and to the current `rotatingId`. A captured payload cannot be replayed after 5 minutes and cannot be reused across different rotating-ID windows. |
| **Spam / Volt farming** | Three-layer dedup: 60-min scan / 60-min rotating-ID / 24-hour identity (MAC-based fallback for privacy-mode peers) |
| **Data leakage** | Personal data (name, avatar, greeting) only flows over an active GATT connection to a verified ThunderPass peer — never in the broadcast packet |
| **Privacy mode** | When enabled, the GATT payload replaces all personal fields with `"Private User"`; BLE advertising continues normally so you still collect passes without revealing identity |
| **No account required** | Every device receives an **anonymous Supabase session** automatically on first launch — no email, no password, no prompts. This supplies a verified server UUID for encounter deduplication. Optionally linking a Google or email account adds cross-device sync and protects data against reinstalls. |
| **Server-side data** | Supabase stores only: display name, avatar seed, greeting, Volts total, sticker list, RA username, device type, and last-seen timestamp. No location, no contacts, no message content. |
