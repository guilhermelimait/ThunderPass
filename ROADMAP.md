...
-[x] add the app management on the settings area as well as any other that might be missing and is used by the app.
...
-[x] update the badges view, use the shield that was on it, and remove the edge large as it was created on the previous change
-[x] on profile, show the badges that were achieved, and respect their colors
-[x] share id on profile, make it clear, respect the colors of the app if on darkmode or not, but add a button to share and user decide whatever it uses on the device to share, whatsapp, copy, email, whatever
...
-[x] ra api key is not being saved to the local profile, and if the installation is updated, it is losing the profile information
